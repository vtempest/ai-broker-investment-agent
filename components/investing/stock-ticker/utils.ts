...
export async function fetchTickerData(symbols: string[]): Promise<TickerData[]> {
  try {
    // Use 5-minute cache for banner (300000ms = 5 minutes)
    const response = await fetch(`/api/stocks/quotes?symbols=${symbols.join(",")}&cacheTTL=300000`)
    ...
    return result.data.map((quote: any) => {
      ...
      const name = symbolNameMap.get(symbol) || quote.shortName || quote.longName || "Unknown"

      // Determine type: indices start with ^, commodities end with =F
      const type = (symbol.startsWith("^") || symbol.endsWith("=F")) ? "index" : "stock"
      ...
      return {
        ...
        weeklyChange: weeklyChange !== undefined ? Number(weeklyChange.toFixed(2)) : undefined,
